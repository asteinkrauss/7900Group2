...
```
```{r}
# Combine the data for different years with an additional 'Year' column
all_fruits <- bind_rows(
  mutate(Fruit2013, Year = "2013"),
  mutate(Fruit2016, Year = "2016"),
  mutate(Fruit2020, Year = "2020")
)

# Sort and select the top 7 most and least expensive fruits from 2013
top_7_2013_fruit <- Fruit2013 %>%
  arrange(desc(CupEquivalentPrice)) %>%
  slice(1:7) %>%
  select(Fruit, Form)

bottom_7_2013_fruit <- Fruit2013 %>%
  arrange(CupEquivalentPrice) %>%
  slice(1:7) %>%
  select(Fruit, Form)

# Create unique identifiers and categories
top_7_2013_fruit <- mutate(top_7_2013_fruit, ID = paste(Fruit, Form, sep = " - "), Category = "Top 7 Most Expensive")
bottom_7_2013_fruit <- mutate(bottom_7_2013_fruit, ID = paste(Fruit, Form, sep = " - "), Category = "Top 7 Least Expensive")

# Combine top and bottom for easier tracking
selected_fruits <- bind_rows(top_7_2013_fruit, bottom_7_2013_fruit)

# Filter and prepare all years' data to include only the selected fruits
tracked_fruits <- all_fruits %>%
  mutate(ID = paste(Fruit, Form, sep = " - ")) %>%
  filter(ID %in% selected_fruits$ID) %>%
  left_join(selected_fruits, by = "ID") %>%
  arrange(ID, desc(Year))

# Plotting the top 5 most expensive fruits
plot_top <- ggplot(filter(tracked_fruits, Category == "Top 7 Most Expensive"), 
                   aes(y = ID, x = CupEquivalentPrice, fill = as.factor(Year))) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.7), orientation = "y") +
  scale_fill_manual(values = c("2013" = "coral", "2016" = "lightgreen", "2020" = "skyblue"),
                    name = "Year") +
  labs(title = "Top 7 Most Expensive Fruits (2013-2020)",
       y = "Fruit - Form",
...
# Plotting the top 5 least expensive fruits
plot_bottom <- ggplot(filter(tracked_fruits, Category == "Top 7 Least Expensive"), 
                      aes(y = ID, x = CupEquivalentPrice, fill = as.factor(Year))) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.7), orientation = "y") +
  scale_fill_manual(values = c("2013" = "coral", "2016" = "lightgreen", "2020" = "skyblue"),
                    name = "Year") +
  labs(title = "Top 7 Least Expensive Fruits (2013-2020)",
       y = "Fruit - Form",
       x = "Cup Equivalent Price ($)") +
  theme_minimal() +
  theme(axis.text.y = element_text(angle = 0, hjust = 1),
...
# Combine the data for different years with an additional 'Year' column
all_vegs <- bind_rows(
  mutate(Veg2013, Year = "2013"),
  mutate(Veg2016, Year = "2016"),
  mutate(Veg2020, Year = "2020")
)

# Sort and select the top 7 most and least expensive Vegetables from 2013
top_7_2013_veg <- Veg2013 %>%
  arrange(desc(CupEquivalentPrice)) %>%
  slice(1:7) %>%
  select(Vegetable, Form)

bottom_7_2013_veg <- Veg2013 %>%
  arrange(CupEquivalentPrice) %>%
  slice(1:7) %>%
  select(Vegetable, Form)

# Create unique identifiers and categories
top_7_2013_veg <- mutate(top_7_2013_veg, ID = paste(Vegetable, Form, sep = " - "), Category = "Top 7 Most Expensive")
bottom_7_2013_veg <- mutate(bottom_7_2013_veg, ID = paste(Vegetable, Form, sep = " - "), Category = "Top 7 Least Expensive")

# Combine top and bottom for easier tracking
selected_vegs <- bind_rows(top_7_2013_veg, bottom_7_2013_veg)

# Filter and prepare all years' data to include only the selected vegetables
tracked_vegs <- all_vegs %>%
  mutate(ID = paste(Vegetable, Form, sep = " - ")) %>%
  filter(ID %in% selected_vegs$ID) %>%
  left_join(selected_vegs, by = "ID") %>%
  arrange(ID, desc(Year))

# Plotting the top 7 most expensive vegetables
plot_top <- ggplot(filter(tracked_vegs, Category == "Top 7 Most Expensive"), 
                   aes(y = ID, x = CupEquivalentPrice, fill = as.factor(Year))) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.7), orientation = "y") +
  scale_fill_manual(values = c("2013" = "coral", "2016" = "lightgreen", "2020" = "skyblue"),
                    name = "Year") +
  labs(title = "Top 7 Most Expensive Vegetables (2013-2020)",
       y = "Vegetable - Form",
       x = "Cup Equivalent Price ($)") +
  theme_minimal() +
  theme(axis.text.y = element_text(angle = 0, hjust = 1),
...
# Plotting the top 5 least expensive fruits
plot_bottom <- ggplot(filter(tracked_vegs, Category == "Top 7 Least Expensive"), 
                      aes(y = ID, x = CupEquivalentPrice, fill = as.factor(Year))) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.7), orientation = "y") +
  scale_fill_manual(values = c("2013" = "coral", "2016" = "lightgreen", "2020" = "skyblue"),
                    name = "Year") +
  labs(title = "Top 7 Least Expensive Vegetables (2013-2020)",
       y = "Vegetable - Form",
       x = "Cup Equivalent Price ($)") +
  theme_minimal() +
  theme(axis.text.y = element_text(angle = 0, hjust = 1),
...
g <- ggplot(map_income) +
  ...
  ggtitle("Personal Income by State (2013, 2016, & 2020)")
...
```
```{r}
# Only include these state names
state_names <- c("Alabama", "Alaska", "Arizona", "Arkansas", "California", "Colorado", "Connecticut", 
                 ...
                 "Wyoming")
# Filter the income data for the specified state names
filtered_income_2013 <- filter(income_2013, GeoName %in% state_names)
filtered_income_2016 <- filter(income_2016, GeoName %in% state_names)
filtered_income_2020 <- filter(income_2020, GeoName %in% state_names)

# Function to calculate wage growth for each state
calculate_wage_growth <- function(income_2013, income_2020) {
  wage_growth <- ((income_2020 - income_2013) / income_2013) * 100
  return(wage_growth)
}

# Create a dataframe to store the results
income_state <- data.frame(State = character(),
                            Average_Income_2013 = numeric(),
                            Average_Income_2020 = numeric(),
                            Wage_Growth_2013_2020 = numeric(),
                            stringsAsFactors = FALSE)

# Iterate over each state to calculate wage growth
for (state in state_names) {
  # Get incomes for the current state
  income_state_2013 <- filter(filtered_income_2013, GeoName == state)$Value
  income_state_2020 <- filter(filtered_income_2020, GeoName == state)$Value
  
  # Calculate wage growth for the current state
  wage_growth <- calculate_wage_growth(income_state_2013, income_state_2020)
  
  # Append results to the dataframe
  income_state <- rbind(income_state,
                        data.frame(State = state,
                                   Average_Income_2013 = income_state_2013,
                                   Average_Income_2020 = income_state_2020,
                                   Wage_Growth_2013_2020 = wage_growth,
                                   stringsAsFactors = FALSE))
}
...
# Calculate the nominal wage for each state in 2013 and 2020
income_state <- mutate(income_state,
                       Nominal_Wage_2013 = Average_Income_2013 * (cpi_2020 / cpi_2013),
                       Nominal_Wage_2020 = Average_Income_2020)

# Keep only necessary columns
income_state <- select(income_state, State, Nominal_Wage_2013, Nominal_Wage_2020)

# Calculate wage growth percentage for each state
income_state <- mutate(income_state,
                       Wage_Growth_Percentage = ((Nominal_Wage_2020 - Nominal_Wage_2013) / Nominal_Wage_2013) * 100)

# Display the updated income_state dataframe
print(income_state)
```
```{r}
# Calculate the percentage change in nominal wage from 2013 to 2020
income_state <- mutate(income_state,
                       Wage_Change = ((Nominal_Wage_2020 - Nominal_Wage_2013) / Nominal_Wage_2013) * 100)

# Calculate the difference between each state's wage growth rate and the inflation rate
income_state <- mutate(income_state,
                       Wage_Inflation_Difference = Wage_Change - 11.1)

# Merge the classified data with the map data
map_income_state <- merge(us_states, income_state, by.x = "full", by.y = "State")

# Plotting
g <- ggplot(map_income_state) +
  geom_sf(aes(fill = Wage_Inflation_Difference)) +
  scale_fill_gradient2(low = "red3", mid = "white", high = "green3", midpoint = 0, 
                       name = "Wage-Inflation Difference (%)") +
  ggtitle("Wage Growth vs. Inflation Rate (2013-2020)")
...
interactive_plot_fruit_form <- ggplotly(ggplot(avg_prices_fruit, aes(x = Year, y = CupEquivalentPrice, group = Fruit, color = Form)) +
                                geom_line() +
                                geom_point() +
                                labs(x = "Year", y = "Retail Price (per equivalent cup, $)",
...
plot_average_price_by_form <- function(data, title) {
  ...
  p <- ggplot(data, aes(x = Year, y = mean_price, color = Form, group = Form)) +
    geom_line() +
    geom_point() +
